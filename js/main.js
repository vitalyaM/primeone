...
function adaptive_header(w,h) {
	...
	if(w<767.98){
		if(!$('.header-body-nav').hasClass('done')){
			$('.header-body-nav').addClass('done').appendTo(headerMenu);
		}
	}else{
		$.each($('.header-body-nav'), function(index, val) {
			if($(this).hasClass('header-body-nav--right')){
				if($(this).hasClass('done')){
					$(this).removeClass('done').prependTo($('.header-body__column').eq(2));
				}
			}else{
				if($(this).hasClass('done')){
					$(this).removeClass('done').prependTo($('.header-body__column').eq(0));
				}
			}
		});
	}
...
function map(n){
	...
	function init(){ 
		...
		var myMap = new ymaps.Map("map", {
			...
			controls: [],
			center: [40.742918,-73.926063],
			// Уровень масштабирования. Допустимые значения:
			// от 0 (весь мир) до 19.
			zoom: 15
		});
		
		myPlacemar = new ymaps.Placemark([40.742918,-73.926063],{
			id:'2'
		},{
			// Опции.
			hasBalloon:false,
